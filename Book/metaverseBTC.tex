...
\noindent \textsc{Flossverse (Free/Libre Open Source Software Metaverse Applications - A Bitcoin First initiative.}\\ % Publisher

\noindent \textsc{\href{https://raw.githubusercontent.com/Flossverse/Metaverse/main/Paper/metaverseBTC.pdf}{Raw GitHub hyperlink}}\\ % URL
...
\noindent \textit{First printing, March 2022, this version is forked from earlier opensrouce work.} % Printing/edition date
...
\textbf{The book was pushed to main on github as ``minimally complete'', and since then this draft version has re-entered a period of overhaul. The number of stubs and errors are increasing as the net gets recast to capture more `stuff'. The real interesting work of combining the primitives into a new direction for integration of Bitcoin and mixed reality is just beginning. There's new focus for the human / machine interfaced emerging, but not yet in the book. We are happy for the wider community to submit pull requests into github as contributors. There are lots of snags now, be warned.}\
...
\end{figure*}
\chapter{Decentralised internet}
...
\subsection{Constraints at this time}
Current metaverse is handy for one to many broadcasts to younger audiences, because they expect and tolerate a choppy experience. If it's non interactive (like a DJ set), then it effectively adds a spatial dimension to a youtube + chatbox mental model. Seems they are fine with being kicked out and all the bad artifacts because they can quickly rejoin. This needs some product analysis.\par
It's `OK' for few to few, limited controlled telepresence meetings, probably in the range 6-12 people maximum, which is traditional immersive TP as expanded in this book. This is pretty well understood science and doesn't need more research.\par
It's terrible at few to many, where speakers in a round table are all expected to have flawless technology and connections in order for stage management to work. The many are put off immediately by the clunky handoffs when glitches happen. We should avoid few to many use cases.
\begin{itemize}
\subsection{Rough list}
...
\subsection{Metaverse and production XR}
\begin{itemize}
\item use modern production XR infrastructure to connect virtual and real through the wall interface as in research telepresence
\item Persistent backstage universes for monetisation of assets.
\item "backstage" at live / recorded mixed reality gigs
\item Remote director / producer / global technical support telepresence "through" the XR screen itself into live sets
\end{itemize}
